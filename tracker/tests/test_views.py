...
class TestDashboardView:
    ...
    def test_dashboard_loads_successfully(self, db):
        response = self.client.get(reverse('tracker:dashboard'))
        assert response.status_code == 200

    def test_dashboard_root_url_loads(self, db):
        response = self.client.get('/tracker/')
        assert response.status_code == 200

    def test_dashboard_uses_correct_template(self, db):
        response = self.client.get(reverse('tracker:dashboard'))
        assert 'tracker/dashboard.html' in [t.name for t in response.templates]

    def test_dashboard_context_keys(self, db):
        response = self.client.get(reverse('tracker:dashboard'))
    ...
    def test_dashboard_with_no_data(self, db):
        response = self.client.get(reverse('tracker:dashboard'))
        context = response.context
...
class TestImportJsonView:
    ...
    def test_import_json_get_not_allowed(self, authenticated_client, db):
        """Test GET request returns 405."""
        response = authenticated_client.get(reverse('tracker:import_json'))
        assert response.status_code == 405

    def test_import_json_empty_data(self, authenticated_client, db):
        """Test import with empty data returns error."""
        response = authenticated_client.post(reverse('tracker:import_json'), {'json_data': ''})
        data = response.json()

        assert data['success'] is False
        assert 'No JSON data provided' in data['error']

    def test_import_json_invalid_json(self, authenticated_client, db):
        """Test import with invalid JSON returns error."""
        response = authenticated_client.post(reverse('tracker:import_json'), {'json_data': 'not valid json'})
        data = response.json()

        assert data['success'] is False
        assert 'Invalid JSON' in data['error']

    def test_import_json_valid_dietary(self, authenticated_client, db, user):
        ...
        response = authenticated_client.post(reverse('tracker:import_json'), {'json_data': json_data})
        data = response.json()
    ...
    def test_import_json_valid_exercise(self, authenticated_client, db, user):
    ...
    def test_import_json_food_key_alias(self, authenticated_client, db, user):
    ...
    def test_import_json_note_key_alias(self, authenticated_client, db, user):
    ...
    def test_import_json_duration_min_alias(self, authenticated_client, db, user):
    ...
    def test_import_json_day_remarks_propagation(self, authenticated_client, db, user):
    ...
    def test_import_json_entry_remarks_override(self, authenticated_client, db, user):
    ...
    def test_import_json_multiple_days(self, authenticated_client, db, user):
        ...
        assert data['success'] is True
        assert '2 dietary' in data['message']

    def test_import_json_creates_entries_for_logged_in_user(self, authenticated_client, db, user):
        """Test import creates entries for the logged-in user."""
        json_data = json.dumps([{
            "date": str(date.today()),
        ...
        assert DietaryEntry.objects.filter(item='Test', user=user).exists()

    def test_import_json_skips_entries_without_date(self, authenticated_client, db, user):
...
class TestAddWeightView:
    ...
    def test_add_weight_get_not_allowed(self, authenticated_client, db):
        """Test GET request returns 405."""
        response = authenticated_client.get(reverse('tracker:add_weight'))
        assert response.status_code == 405

    def test_add_weight_missing_weight(self, authenticated_client, db):
        """Test add weight with missing weight returns error."""
        response = authenticated_client.post(reverse('tracker:add_weight'), {'weight_kg': ''})
        data = response.json()

        assert data['success'] is False
        assert 'Weight is required' in data['error']

    def test_add_weight_valid(self, authenticated_client, db, user):
        """Test adding valid weight entry."""
        response = authenticated_client.post(reverse('tracker:add_weight'), {
            'weight_kg': '75.5',
            'date': str(date.today()),
        ...
        assert WeightEntry.objects.filter(weight_kg=Decimal('75.5'), user=user).exists()

    def test_add_weight_without_date_uses_today(self, authenticated_client, db, user):
    ...
    def test_add_weight_without_notes(self, authenticated_client, db, user):
    ...
    def test_add_weight_creates_entry_for_logged_in_user(self, authenticated_client, db, user):
        """Test add weight creates entry for the logged-in user."""
        response = authenticated_client.post(reverse('tracker:add_weight'), {
            'weight_kg': '68.5',
            'date': str(date.today())
        })

        assert WeightEntry.objects.filter(weight_kg=Decimal('68.5'), user=user).exists()

    def test_add_weight_invalid_decimal(self, authenticated_client, db, user):
...
class TestDailyRecapView:
    ...
    def test_daily_recap_returns_json(self, authenticated_client, db):
        """Test daily recap returns JSON response."""
        response = authenticated_client.get(reverse('tracker:daily_recap', args=[str(date.today())]))
        assert response['Content-Type'] == 'application/json'

    def test_daily_recap_empty_date(self, authenticated_client, db):
        """Test daily recap for date with no entries."""
        response = authenticated_client.get(reverse('tracker:daily_recap', args=['2020-01-01']))
        data = response.json()
    ...
    def test_daily_recap_with_dietary(self, authenticated_client, dietary_entry):
        """Test daily recap includes dietary entries."""
        response = authenticated_client.get(reverse('tracker:daily_recap', args=[str(dietary_entry.date)]))
        data = response.json()
    ...
    def test_daily_recap_with_exercise(self, authenticated_client, exercise_entry):
        """Test daily recap includes exercise entries."""
        response = authenticated_client.get(reverse('tracker:daily_recap', args=[str(exercise_entry.date)]))
        data = response.json()
    ...
    def test_daily_recap_with_weight(self, authenticated_client, weight_entry):
        """Test daily recap includes weight entries."""
        response = authenticated_client.get(reverse('tracker:daily_recap', args=[str(weight_entry.date)]))
        data = response.json()
    ...
    def test_daily_recap_summary_calculations(self, authenticated_client, db, user):
        ...
        response = authenticated_client.get(reverse('tracker:daily_recap', args=[str(today)]))
        data = response.json()
    ...
    def test_daily_recap_invalid_date_format(self, authenticated_client, db):
        """Test daily recap with invalid date format."""
        response = authenticated_client.get(reverse('tracker:daily_recap', args=['invalid-date']))
        data = response.json()

        assert data['success'] is False

    def test_daily_recap_handles_null_calories_burned(self, authenticated_client, db, user):
